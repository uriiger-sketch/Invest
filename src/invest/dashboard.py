"""Streamlit dashboard. Four pages: Top 13, Drill-down, Sources & freshness, Methodology."""
...
_HORIZON_LABELS: dict[str, str] = {
    ...
    "daily": "Daily (~5 d)",
    "weekly": "Weekly (~1 mo)",
    "monthly": "Month and above (~90 d)",
}


def _horizon_label(h: str) -> str:
    return _HORIZON_LABELS.get(h, h)
...
page = st.sidebar.radio(
    "Page",
    ("Top 13", "Ticker drill-down", "Sources & freshness", "Methodology"),
)
...
if page == "Top 13":
    st.title("Top 13 stocks to invest in")
    if as_of is None:
        st.stop()
